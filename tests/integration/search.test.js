...
describe('/api/v1/search', () => {
  ...
  describe('GET /', () => {
    ...
    it('should get all the document if the the admin requests for it and the accessRight and role query parameter are valid and invalid respectively', async () => {
      const response = await request(server)
        .get('/api/v1/search?accessRight=1&role=admin1')
        .set('x-auth-token', adminToken)
        .send();

      expect(response.status).toBe(400);
    });
    it('should get all the document if the the admin requests for it and the role and accessRight query parameter are valid and invalid respectively', async () => {
      const response = await request(server)
        .get('/api/v1/search?accessRight=5&role=admin')
        .set('x-auth-token', adminToken)
        .send();
